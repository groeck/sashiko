...
## 1. Problem Statement
The current agent implementation relies on a simple, hard-coded loop limit (`MAX_TURNS = 10`). This is insufficient for complex tasks like "deep dive regression analysis," which may require:
...
1.  **Infinite Loops**: The agent getting stuck repeating the same action (e.g., `ls` -> `ls` -> `ls`).
2.  **Cost Overruns**: Excessive token usage if the agent "hallucinates" a never-ending investigation.
...
The agent must generate "Findings" or "New Information".
- If the agent performs 5 tool calls but the "Findings" list hasn't grown (or the internal "Thought" trace doesn't indicate a new hypothesis), we increment a `stalled_turns` counter.
...
1.  **`search_code` (ripgrep)**: Instead of `list_dir` + `read_file` loops to find a symbol, the agent can grep the entire tree in 1 step.
...
### 3.1. Modify `Agent` Struct
Refactor `src/agent/mod.rs`:
...
*Smart Agent*:
1. `search_file_content "loops"` -> Returns matches in `src/agent/mod.rs` and `DESIGN_LOOP_PREVENTION.md`.
2. `read_file src/agent/mod.rs`
3. Done. (3 steps)

### Scenario B: The "Stuck" Agent
*Agent*:
1. `git_diff HEAD`
